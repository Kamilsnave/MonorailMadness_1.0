...
public class TacGnol extends EnemyEntity{

	private Sprite sprite;
	
	public TacGnol(GameScene gameScene, int layer) {
		...
		setVelocity(new Vector2f(0, 400.0f));
		setAcceleration(new Vector2f(0, 100));
		setHitBoxOffset(new Vector2f(0, 0));
		setHitBoxSize(getSize());
		
		sprite = new Sprite("TACGNOL");
		sprite.setDimensions(getSize().getX(), getSize().getY());
		
		mediaPlayer = ResourceManager.getAudio("DEATH3");
	}
